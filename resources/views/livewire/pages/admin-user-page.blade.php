<div x-data="{ tab: 'prodi' }" class="mx-auto max-w-[1440px] space-y-6">
    <div class="flex flex-wrap items-end justify-between gap-4">
        <div>
            ...
            <h1 class="text-2xl font-extrabold tracking-tight text-zinc-900">Program Studi &amp; Akses Pengguna</h1>
            <p class="mt-1 text-sm text-zinc-500">Buat prodi baru, tambah akun akses, dan kelola pengguna dari halaman ini.</p>
        </div>
    ...
    <div class="rounded-3xl border border-zinc-200 bg-white p-2 shadow-sm">
        <div class="grid gap-2 md:grid-cols-3">
            <button type="button" @click="tab = 'prodi'"
                :class="tab === 'prodi' ? 'bg-indigo-600 text-white shadow-sm' : 'text-zinc-600 hover:bg-zinc-50 hover:text-zinc-900'"
            ...
            <button type="button" @click="tab = 'akun'"
                :class="tab === 'akun' ? 'bg-indigo-600 text-white shadow-sm' : 'text-zinc-600 hover:bg-zinc-50 hover:text-zinc-900'"
                class="flex items-center justify-center gap-2 rounded-2xl px-4 py-3 text-sm font-bold transition">
                <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5"><path stroke-linecap="round" stroke-linejoin="round" d="M18 9v3m0 0v3m0-3h3m-3 0h-3m-2-6a4 4 0 11-8 0 4 4 0 018 0zM3 20a6 6 0 0112 0v1H3v-1z"/></svg>
                Akun Tambahan
            </button>
            <button type="button" @click="tab = 'daftar'"
                :class="tab === 'daftar' ? 'bg-indigo-600 text-white shadow-sm' : 'text-zinc-600 hover:bg-zinc-50 hover:text-zinc-900'"
                class="flex items-center justify-center gap-2 rounded-2xl px-4 py-3 text-sm font-bold transition">
                <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5"><path stroke-linecap="round" stroke-linejoin="round" d="M17 20h5v-2a4 4 0 00-4-4h-1M9 20H4v-2a4 4 0 014-4h1m4-4a4 4 0 100-8 4 4 0 000 8zm6 0a3 3 0 100-6 3 3 0 000 6z"/></svg>
                Daftar Pengguna
            </button>
        </div>
    </div>

    <section x-show="tab === 'prodi'" x-transition class="overflow-hidden rounded-3xl border border-indigo-100 bg-white shadow-sm">
        <div class="flex items-start gap-3 border-b border-indigo-100 bg-gradient-to-r from-indigo-50 via-blue-50/60 to-white px-6 py-5">
            ...
            <div>
                <h2 class="text-base font-extrabold text-zinc-900">Tambah Prodi Baru + Akun Kaprodi</h2>
                <p class="mt-1 text-xs leading-relaxed text-zinc-500">Gunakan tab ini hanya untuk program studi baru. Akun Kaprodi pertama dibuat otomatis dari data di bawah.</p>
            </div>
        </div>
        <form wire:submit="createProdiWithKaprodi" class="grid gap-x-5 gap-y-4 p-6 sm:grid-cols-2 xl:grid-cols-3">
            ...
            <div class="flex items-center justify-between gap-4 border-t border-zinc-100 pt-4 sm:col-span-2 xl:col-span-3">
                <p class="hidden text-xs text-zinc-400 sm:block">Username boleh memakai titik, contoh: kaprodi.ekonomi.</p>
                <button class="adm-btn-primary w-full sm:w-auto" type="submit" wire:loading.attr="disabled">
                    ...
                    <span wire:loading.remove wire:target="createProdiWithKaprodi">Simpan Prodi &amp; Kaprodi</span>
    ...
    <section x-show="tab === 'akun'" x-transition class="rounded-3xl border border-zinc-200 bg-white p-6 shadow-sm">
        <div class="mb-5 flex items-start gap-3 border-b border-zinc-100 pb-4">
            ...
            <div>
                <h2 class="text-base font-extrabold text-zinc-900">{{ $editingId ? 'Edit Akun' : 'Tambah Akun Tambahan' }}</h2>
                <p class="mt-1 text-xs leading-relaxed text-zinc-500">Gunakan tab ini hanya jika program studi sudah ada.</p>
            </div>
        </div>
        <form wire:submit="save" class="space-y-4">
            <label class="adm-field-label">Nama Lengkap
                <input wire:model="name" class="adm-input @error('name') is-invalid @enderror" placeholder="Nama lengkap pengguna">
                @error('name')<span class="mt-1 block text-xs font-medium text-rose-600">{{ $message }}</span>@enderror
            </label>
            <label class="adm-field-label">Username
                <input wire:model="username" class="adm-input @error('username') is-invalid @enderror" placeholder="Contoh: sekprodi.ekonomi">
                @error('username')<span class="mt-1 block text-xs font-medium text-rose-600">{{ $message }}</span>@enderror
            </label>
            <label class="adm-field-label">Email
                <input wire:model="email" type="email" class="adm-input @error('email') is-invalid @enderror" placeholder="nama@unwari.ac.id">
                @error('email')<span class="mt-1 block text-xs font-medium text-rose-600">{{ $message }}</span>@enderror
            </label>
            <div class="grid gap-4 md:grid-cols-2">
                <label class="adm-field-label">Role
                    <select wire:model.live="role" class="adm-input @error('role') is-invalid @enderror">
                        <option value="sekprodi">Sekprodi</option>
                        <option value="kaprodi">Kaprodi</option>
                        <option value="admin">Admin</option>
                    </select>
                    @error('role')<span class="mt-1 block text-xs font-medium text-rose-600">{{ $message }}</span>@enderror
                </label>
                @if ($role !== 'admin')
                    <label class="adm-field-label">Program Studi
                        <select wire:model="prodi_id" class="adm-input @error('prodi_id') is-invalid @enderror">
                            @foreach($prodis as $prodi)
                                <option value="{{ $prodi->id }}">{{ $prodi->code }} - {{ $prodi->name }}</option>
                            @endforeach
                        </select>
                        @error('prodi_id')<span class="mt-1 block text-xs font-medium text-rose-600">{{ $message }}</span>@enderror
                    </label>
                @else
                    <div class="rounded-2xl border border-zinc-100 bg-zinc-50/70 p-4 text-sm text-zinc-500">
                        Admin tidak perlu dipilihkan Program Studi. Akun akan otomatis dipasang ke akses sistem pusat.
                    </div>
                @endif
            </div>
            <label class="adm-field-label">Password
                <input wire:model="password" type="password" class="adm-input @error('password') is-invalid @enderror" placeholder="{{ $editingId ? 'Kosongkan jika tidak diganti' : 'Minimal 8 karakter' }}">
    ...
    <section x-show="tab === 'daftar'" x-transition class="overflow-hidden rounded-3xl border border-zinc-200 bg-white shadow-sm">
        <div class="flex items-center justify-between gap-3 border-b border-zinc-200 px-5 py-4">
            <div>
                ...
                <p class="mt-1 text-xs text-zinc-500">{{ $users->count() }} akun terdaftar</p>
            </div>
                        ...
                        <tr class="border-b border-zinc-100 last:border-0 hover:bg-zinc-50/70">
                            <td class="px-5 py-4">
                                <div class="flex items-center gap-3">
                                    <div class="grid h-9 w-9 shrink-0 place-items-center rounded-full bg-indigo-100 text-xs font-extrabold text-indigo-700">{{ $user->initials() }}</div>
                                    <div class="min-w-0">
                                        <div class="truncate font-bold text-zinc-900">{{ $user->name }}</div>
                                        <div class="mt-0.5 truncate text-xs text-zinc-500">&#64;{{ $user->username }} <span class="text-zinc-300">-</span> {{ $user->email }}</div>
                                    </div>
                                </div>
                            </td>
                            ...
                            <td class="px-5 py-4 text-right">
                                <div class="inline-flex items-center gap-1">
                                    <button wire:click="edit({{ $user->id }})" @click="tab = 'akun'" class="rounded-lg p-2 text-zinc-500 transition hover:bg-indigo-50 hover:text-indigo-700" title="Edit akun">
                                        <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                                    </button>
                                    @if($user->email !== env('ADMIN_EMAIL', 'admin@prodi.local'))
                                        <button wire:click="delete({{ $user->id }})" wire:confirm="Hapus akun ini?" class="rounded-lg p-2 text-zinc-500 transition hover:bg-rose-50 hover:text-rose-600" title="Hapus akun">
                                            <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M6 7h12m-9 0v10m6-10v10M9 7V4h6v3m-9 0h12l-1 13H7L6 7z"/></svg>
                                        </button>
                                    @endif
                                </div>
                            </td>
